...
from libs_python.payload_sdk import PayloadSdkInterface, payload_status_event_t, param_type, capture_sequence_t
...
def onPayloadStatusChanged(event: int, param: list):
    ...
    if event == payload_status_event_t.PAYLOAD_CAM_CAPTURE_STATUS:
        ...
        elif my_capture == capture_sequence_t.STOP_CAPTURING_IMAGE:
            if param[0] == 0: 
                my_capture = capture_sequence_t.CHECK_STORAGE
        ...
        if my_capture == capture_sequence_t.CHECK_CAMERA_MODE:
            ...
            if param[0] == camera_mode.CAMERA_MODE_IMAGE:
...
def main():
    ...
    while not time_to_exit:
        ...
        if my_capture == capture_sequence_t.IDLE:
            ...
            my_capture = capture_sequence_t.IMAGE_IN_CAPTURING

        elif my_capture == capture_sequence_t.IMAGE_IN_CAPTURING:
            print("Captured ...")
            time.sleep(interval)  
            time_to_capturing -= interval
            if time_to_capturing <= 0:
                my_capture = capture_sequence_t.STOP_CAPTURING_IMAGE
