...
def onPayloadStatusChanged(event: int, param: list):
    ...
    if event == payload_status_event_t.PAYLOAD_CAM_CAPTURE_STATUS.value:      
        ...
        elif my_capture == capture_sequence_t.WAIT_CAPTURE_DONE.value:
            if param[0] == 0:
                image_to_capture -= 1
                my_capture = capture_sequence_t.CHECK_STORAGE.value
...
def main():
    ...
    my_payload.setPayloadCameraParam(PAYLOAD_CAMERA_RECORD_SRC, Payload_Camera_Record_Src.PAYLOAD_CAMERA_RECORD_EO.value, param_type.PARAM_TYPE_UINT32.value)
    
    # Set photo storage to Internal
    if PAYLOAD_TYPE == "GHADRON":
        my_payload.setPayloadCameraParam(PAYLOAD_CAMERA_STORAGE, Payload_Camera_Storage.PAYLOAD_CAMERA_STORAGE_INTERNAL.value, param_type.PARAM_TYPE_UINT32.value)
    ...
    while not time_to_exit:
        ...
        if my_capture == capture_sequence_t.IDLE.value:
            ...
            my_capture = capture_sequence_t.WAIT_CAPTURE_DONE.value   
