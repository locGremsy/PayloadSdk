...
def receive_messages():
    ...
    while not is_exit:
        msg_cnt, msg = my_payload.getNewMessage()
        if msg_cnt:
            raw_bytes = bytes(ctypes.string_at(ctypes.addressof(msg), 20))
            sdk_log("receive_messages", f"Raw bytes: {raw_bytes.hex()}")
            msgid = msg.get_msgid()
            sdk_log("receive_messages", f"Received message: msgid={msgid}, sysid={msg.sysid}, compid={msg.compid}")
            if msgid == MAVLINK_MSG_ID_COMMAND_ACK:  # 77
                payload = ctypes.cast(msg.payload64, ctypes.POINTER(ctypes.c_uint8))
                cmd_id = ctypes.c_uint16.from_buffer(payload, 0).value
                result = payload[2]
                progress = payload[3]
                sdk_log("receive_messages", f"COMMAND_ACK parsed: cmd_id={cmd_id}, result={result}, progress={progress}")
                on_payload_status_changed(PAYLOAD_ACK, [cmd_id, result, progress])
